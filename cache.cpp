...
constexpr inline Index get_hash(Index key_hash) {
	//we want to mark entries using EMPTY and DELETED as their key_hash
	//we need to modify the hash so it can't equal EMPTY or DELETED
...
constexpr inline Index get_step_size(Index key_hash) {
	...
	//so it must be odd(if it's not it may crash)
	//we want the step size to have little relation with the initial index
	return ~((key_hash<<2)|HIGH_BIT);//negating the high bit to avoid overflows
}
...
void cache_set(Cache* cache, const Key_ptr key, const Value_ptr val, const Index val_size) {
	...
	byte* val_copy = new byte[val_size];//assuming val_size is in mem_units
